                ...
                api.put(`Customers/editCustomer/${res.userId}?access_token=${res.id}`, { deviceToken: this.state.deviceToken }).then((resEdit) => {
                  ...
                  this.props.getUserDetail(res.userId, res.id).then((userRes) => {
                    if (this.props.auth.data.language) {
                      const langData = { langId: this.props.auth.data.languageId, language: this.props.auth.data.language.languageName, Code: this.props.auth.data.language };
                      AsyncStorage.setItem("language", JSON.stringify(langData)).then((langSuccess) => {
